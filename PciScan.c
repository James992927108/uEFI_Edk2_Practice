...
UINTN PciParseBar(IN EFI_PCI_IO_PROTOCOL *PciIo, IN UINTN Offset, IN UINTN BarIndex)
{
    UINT32 Value = 0;
...
EFI_STATUS EFIAPI ReadPci(EFI_HANDLE *PciIoHandleBuffer, UINTN index, UINTN TotalDevice)
{
    ...
    Status = PciIo->Pci.Read(PciIo, EfiPciWidthUint32, 0, sizeof(PCI_TYPE_GENERIC) / sizeof(UINT32), &PciData);
    if (EFI_ERROR (Status))
        return Status;
    ...
    case HEADER_TYPE_DEVICE:
        ...
        Print(L"=> DEVICE\n");
        for (UINTN Offset = 0x10, BarIndex = 0; Offset <= 0x24; BarIndex++)
        {
            Offset = PciParseBar(PciIo, Offset, BarIndex);
        }
    ...
    case HEADER_TYPE_PCI_TO_PCI_BRIDGE:
        ...
        for (UINTN Offset = 0x10, BarIndex = 0; Offset <= 0x14; BarIndex++)
        {
            Offset = PciParseBar(PciIo, Offset, BarIndex);
        }
    ...
    else
    {
        ...
        PciParseBar(PciIo, PCI_EXPANSION_ROM_BASE, 0);
    }
}
